...
public class UserResource {
	...
	public List<Map<String, String>> getAll(@QueryParam("q") String q, @QueryParam("order") String order,
		...
		{
			List<User> result = new ArrayList<>(), models = new ArrayList<>(repository.getAllUser()); // No se puede utilizar .toList() porque eso es a partir de Java 16.
			int start = offset == null ? 0: offset-1; // Donde va a comenzar.
			int end = limit == null ? models.size(): start  + limit; // Donde va a terminar.
			if (q != null)
				for (int i = start; i < end; i++) {
					// Comprobamos que contiene la cadena q y el resto de restricciones.
					if (contieneQ(models.get(i), q) /* && resto restricciones*/)
	            ...
	            return result.stream().map(model -> model.getFields((fields == null) ? "id, name, decription, songs": fields)).collect(Collectors.toList());;
	        }
	    }

	    private void orderResult(List<User> result, String order) {
	        // if (order.equals("atributo")) Collections.sort(result,Comparator.comparing(Model::getAtributo))
	    }

	    @GET
	    @Path("/{id}")
	    @Produces("application/json")
	    public User getUser(@PathParam("id") String id) throws NotFoundException /* No debería de ser necesario este throw */ {
